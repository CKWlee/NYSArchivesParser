...
# ─── (1) Locate all decoded CSV files ──────────────────────────────────────────
...
for year in years:
    ...
    # — (6G) BY INSTITUTION × COURT (dedicated columns) —
    if "Institution" in sub.columns and "CourtCommittedByName" in sub.columns and "CourtCommittedBy" in sub.columns:
        ic = sub.groupby(["Institution", "CourtCommittedBy", "CourtCommittedByName"]) \
                .size().reset_index(name="Count")
        for _, row in ic.iterrows():
            inst    = row["Institution"].strip().lower()
            code    = row["CourtCommittedBy"].strip()
            courtnm = row["CourtCommittedByName"].strip().lower()
            if inst and code and courtnm:
                histpun_rows.append({
                    "Country":    "United States",
                    "Year":        int(year),
                    "Statistic":  "prisoners",
                    ...
                    "Crime":       "",
                    "Institution": inst,
                    "CourtCode":   code,    # RAW numeric/letter code
                    "Court":       courtnm, # human‐readable name
                    "Complete":    ""
                })

    # — (6H) BY INSTITUTION ONLY (no Complete, no Court) —
...
# ─── (7) Assemble final DataFrame with exactly these columns ──────────────────
histpun_df = pd.DataFrame(histpun_rows)
histpun_cols = [
    "Country", "Year", "Statistic", "Value", "Source", "State",
    "Race", "Gender", "Age", "Crime", "Institution", "CourtCode", "Court", "Complete"
]
...
# ─── (8) Write out the CSV ─────────────────────────────────────────────────────
output_path = "histpun_inst_court.csv"
